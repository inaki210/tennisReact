  ...
  render() {
    ...
    return (
      <div className="margin-horizontal-small display-flex-column align-items-center">
        <h4 id="lblPlayer">
          {status
            ? this.props.game.getNombreJugador(numeroJugador)
            : 'Player' + numeroJugador}
        </h4>
        {status ? (
          <button
            onClick={() => {
              this.anotarPunto(numeroJugador), this.imprimirResultado()
            }}
            disabled={!!this.props.game.ganador}
          >
            Won point
          </button>
        ) : (
          <TxtJugador
            nombreJugador={this.props.game.getNombreJugador(numeroJugador)}
